  ...
  if (cmd_stdout.length() == 0) {
    ...
    if (err.length() == 0) {
      QMessageBox::information(nullptr, "Error",
                            "Make sure you have at least one 2FA OTP key "
                            "loaded into '2fa' tool.");
      this->on_actionAdd_New_triggered();
      QMessageBox::information(nullptr, "Please restart", "Please restart 2fa-qt");
    } else {
...
void MainWindow::on_actionAdd_New_triggered() {
  ...
  QString text1 =
      QInputDialog::getText(this, tr("QInputDialog::getText()"),
                            tr("2FA key name:"), QLineEdit::Normal, "", &ok1);
  if (!ok1) {
    return;
  }
  QString text2 =
      QInputDialog::getText(this, tr("QInputDialog::getText()"),
                            tr("secret code:"), QLineEdit::Normal, "", &ok2);
  if (ok1 && !text1.isEmpty() && ok2 && !text2.isEmpty()) {
    ...
    QString cmd_stderr = process.readAllStandardError().replace(
        "2fa key for " + text1 + ":", "");
    cmd_stderr = cmd_stderr.trimmed();
    if (process.exitCode() != 0 || cmd_stderr.length() != 0) {
      QMessageBox::critical(nullptr,
                            "Error " + QString::number(process.exitCode()),
                            "Key not added. " + cmd_stderr);
    } else {
      this->repopulate();
    }
  }
